...
df = pd.read_excel("federalfinancegestion/Rennes_DataChallenge2024_Cryptomarkets_dataset.xlsx", index_col=0)
...
# add btc lowercase
columns_to_keep += [col for col in df.columns if 'btc' in col]
columns_to_keep += ["uncertainty_attention_twitter", "cbdc_attention_twitter", "regulation_attention_twitter", "VStoxx", "Gold", "ECRPUS1YIndex", "WTI_CrudeOil", "Brent_CrudeOil","PercentHikeCut"]
...
df["Close_BTC"] = df["Close_BTC"].pct_change()
# shift df["Close_BTC"] to have the rendement of the next day
df["Close_BTC"] = df["Close_BTC"].shift(-1)
# if rendement > 0 then 1 else 0 and if rendement < 0 then -1 else 0
df["Target"] = 0
df.loc[df['Close_BTC'] > 0.02, 'Target'] = 1
df.loc[df['Close_BTC'] < -0.02, 'Target'] = 2
# drop Close_BTC
df = df.drop(columns=["Close_BTC"])
# drop last row
...
grid_search = GridSearchCV(xgb_model, param_grid, cv=3, verbose=1, n_jobs=-1, scoring='f1_weighted')
